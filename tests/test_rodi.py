...
from dij import ActivationScope, Container, ContainerProtocol, ServiceLifeStyle, Services, inject
from dij.exception import (
    AliasAlreadyDefined,
...
from dij.resolver.dynamic import DynamicResolver
from dij.resolver.instance import InstanceResolver
from dij.svc import _get_factory_annotations_or_throw
from dij.utils import to_standard_param_name
...
def test_add_instance_allows_for_circular_classes():
    container = Container()
    container.add_instance(Circle(Circle(None)))  # type: ignore (for testing purposes)
...
def test_add_instance_with_declared_type():
    container = Container()
    container.add_instance(Circle(Circle(None)), declared_class=ICircle)  # type: ignore (for testing purposes)
    provider = container.build_provider()
...
def test_scoped_services_context_used_more_than_once():
    ...
    with context:
        ...
        a.b1.c is first_c  # type: ignore
        a.b2.c is first_c  # type: ignore

    with context:
        a = provider.get(A, context)
        second_c = provider.get(C)
        a.b1.c is second_c  # type: ignore
        a.b2.c is second_c  # type: ignore
...
def test_additional_alias():
    ...
    class UsingAlias:
        def __init__(self, example, settings) -> None:
            self.cats_controller = example
            self.settings = settings

    class AnotherUsingAlias:
        def __init__(self, cats_controller, service_settings) -> None:
...
def test_invalid_factory_too_many_arguments_throws(method_name):
    ...
    def factory(context, activating_type, extra_argument_mistake) -> Cat:  # type: ignore
        return Cat('Celine')

    with raises(InvalidFactory):
        method(factory, Cat)

    def factory(context, activating_type, extra_argument_mistake, two) -> Cat:  # type: ignore
        return Cat('Celine')

    with raises(InvalidFactory):
        method(factory, Cat)

    def factory(context, activating_type, extra_argument_mistake, two, three) -> Cat:
        return Cat('Celine')
...
def test_add_singleton_by_factory_raises_for_missing_type(method_name):
    ...
    def factory(_):  # noqa: ANN202 (no return type on purpose to test failure)
        return Cat('Celine')
...
@pytest.mark.parametrize('method_name', ['add_transient_by_factory', 'add_scoped_by_factory'])
...
def test_raises_invalid_factory_for_non_callable():
    ...
    with raises(InvalidFactory):
        container.register_factory('Not a factory', Cat, ServiceLifeStyle.SINGLETON)  # type: ignore (on purpose error for testing reasons)
...
def test_scoped_services_abstract():
    ...
    assert isinstance(a, A)
    assert isinstance(a.b, B)  # type: ignore
    assert isinstance(a.b.c, C)  # type: ignore

    a2 = provider.get(ABase, context)
    assert a is a2
    assert a.b is a2.b  # type: ignore
    assert a.b.c is a2.b.c  # type: ignore
...
def test_annotations_abstract_type_transient_service():
    class FooCatsRepository(ICatsRepository):
        def get_by_id(self, _id: str) -> Cat:
            return Cat('foo')
...
def test_deps_github_scenario():
    ...
    assert cla_handler.comments_service.auth_handler is cla_handler.checks_service.auth_handler
...
def test_container_protocol():
    ...
    class UsingAlias:
        def __init__(self, example: CatsController):
            self.cats_controller = example
